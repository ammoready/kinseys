...
module Kinseys
  module API
    ...
    ROOT_API_URL = 'https://api.kinseysinc.com/'.freeze
    USER_AGENT = "KinseysRubyGem/#{Kinseys::VERSION}".freeze
    ...
    def process_request(request)
      ...
      response = Net::HTTP.start(uri.host, uri.port, Kinseys.config.proxy_address, Kinseys.config.proxy_port, use_ssl: true) do |http|
        http.request(request)
      end

      Kinseys::Response.new(response)
    end
